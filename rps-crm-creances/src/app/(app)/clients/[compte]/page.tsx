...
import { fmt, fmtF, formatDate, formatDateHeure, libelleMois, aujourdhui, lienWhatsApp, TYPOLOGIES, LIBELLES_TYPE_ACTION, LIBELLES_CANAL, LIBELLES_RESULTAT, LIBELLES_LIEN, LIBELLES_DOCUMENT } from "@/lib/format";
import { Badge, BadgeStatut, Score } from "@/components/Badge";
import { Messages } from "@/components/Messages";
import { creerAction, fermerAction, enregistrerMessage, enregistrerClientExt, qualifierReglement, genererSituation } from "../actions";
import type { VueClient, Ecriture, Livraison, Action, Contact, ModeleMessage } from "@/lib/types";

type SP = { succes?: string; erreur?: string; onglet?: string; modele?: string; page?: string };
...
export default async function PageFicheClient({ params, searchParams }: { params: Promise<{ compte: string }>; searchParams: Promise<SP> }) {
  ...
  await supabase.rpc("journaliser", { p_quoi: "consultation_fiche", p_compte: compte, p_detail: { onglet } });

  // chargement selon l'onglet (3G) ; les écritures servent à la synthèse, aux règlements et à la chronologie
  const besoinEcr = ["synthese", "reglements", "timeline"].includes(onglet);
  const [ecr, facturation, livr, actions, messages, agents, modeles, plans, docs, payeurs] = await Promise.all([
    besoinEcr ? tout<Ecriture>(supabase.from("vue_ecritures").select("*").eq("compte", compte).order("date_ecriture").order("ordre")) : Promise.resolve([] as Ecriture[]),
    onglet === "synthese" ? tout<{ mois: string; ht: number }>(supabase.from("vue_facturation").select("mois, ht").eq("compte", compte).order("mois")) : Promise.resolve([]),
    onglet === "livraisons" || onglet === "synthese" ? tout<Livraison>(supabase.from("vue_livraisons").select("*").eq("compte", compte).order("date_livraison", { ascending: false }).order("ordre", { ascending: false })) : Promise.resolve([] as Livraison[]),
    tout<Action>(supabase.from("actions").select("*").eq("compte", compte).order("cree_le", { ascending: false })),
    onglet === "messages" || onglet === "timeline" ? tout<{ id: string; canal: string; modele: string | null; destinataire: string | null; contenu: string; envoye_par_nom: string | null; envoye_le: string }>(supabase.from("messages_sortants").select("*").eq("compte", compte).order("envoye_le", { ascending: false })) : Promise.resolve([]),
    onglet === "synthese" || onglet === "fiche" ? supabase.from("profils").select("id, nom").eq("actif", true).order("nom").then((r) => r.data ?? []) : Promise.resolve([] as { id: string; nom: string }[]),
    onglet === "messages" ? supabase.from("modeles_messages").select("*").order("niveau").then((r) => (r.data ?? []) as ModeleMessage[]) : Promise.resolve([] as ModeleMessage[]),
    onglet === "synthese" ? supabase.from("plans_echeances").select("*, actions!inner(compte)").eq("actions.compte", compte).order("echeance").then((r) => r.data ?? []) : Promise.resolve([]),
    onglet === "timeline" || onglet === "synthese" ? supabase.from("documents").select("*").eq("compte", compte).order("genere_le", { ascending: false }).limit(10).then((r) => r.data ?? []) : Promise.resolve([]),
    onglet === "reglements" || onglet === "livraisons" ? supabase.from("vue_soldes_payeur").select("*").eq("compte", compte).then((r) => r.data ?? []) : Promise.resolve([] as { payeur: string; bons: number; reglements: number; solde: number }[]),
  ]);
  ...
  const modeleChoisi = modeles.find((m) => m.code === (sp.modele ?? ""));
  let texteMessage = "";
  if (modeleChoisi) {
  ...
  const pieceMulti = new Set(ecr.filter((e) => e.est_reglement).map((e) => `${e.journal}|${e.piece}`));
  const { data: multi } = onglet === "reglements" ? await supabase.from("vue_pieces_multi_clients").select("journal, piece, nb_comptes, total").in("piece", regls.map((e) => e.piece ?? "").filter(Boolean)) : { data: [] };
  const multiSet = new Set((multi ?? []).filter((m) => pieceMulti.has(`${m.journal}|${m.piece}`)).map((m) => `${m.journal}|${m.piece}`));

  type Ev = { date: string; classe: string; titre: string; detail?: string; cle: string };
  const timeline: Ev[] = onglet === "timeline" ? [
    ...actions.map((a) => ({ date: a.cree_le, classe: a.type === "contentieux" || a.type === "mise_en_demeure" ? "rouge" : a.type === "promesse" || a.type === "plan" ? "or" : a.statut === "fermee" ? "gris" : "bleu", cle: "a" + a.id,
      titre: `${LIBELLES_TYPE_ACTION[a.type]}${a.niveau ? ` N${a.niveau}` : ""}${a.montant ? ` — ${fmtF(a.montant)}` : ""}${a.echeance ? ` (échéance ${formatDate(a.echeance)})` : ""} · ${a.statut === "ouverte" ? "ouverte" : `close${a.resultat ? ` : ${LIBELLES_RESULTAT[a.resultat] ?? a.resultat}` : ""}`}`,
      detail: `${a.note ?? ""}${a.ferme_motif ? ` — ${a.ferme_motif}` : ""}${a.reglee_par_piece ? ` (pièce ${a.reglee_par_piece})` : ""} — ${a.auteur}` })),
    ...messages.map((m) => ({ date: m.envoye_le, classe: "bleu", cle: "m" + m.id, titre: `Message ${LIBELLES_CANAL[m.canal] ?? m.canal}${m.modele ? ` (${m.modele})` : ""} → ${m.destinataire ?? "—"}`, detail: `${m.contenu.slice(0, 140)}… — ${m.envoye_par_nom ?? ""}` })),
    ...regls.map((e) => ({ date: e.date_ecriture, classe: e.est_regularisation ? "gris" : "vert", cle: "r" + e.ordre, titre: `${e.est_regularisation ? "Régularisation" : "Règlement"} ${fmtF(e.montant)} — ${e.journal} ${e.piece ?? ""}`, detail: `${e.intitule ?? ""}${e.payeur ? ` · payeur : ${e.payeur}` : ""}${e.lien ? ` · ${LIBELLES_LIEN[e.lien] ?? e.lien}` : ""}` })),
    ...debits.map((e) => ({ date: e.date_ecriture, classe: "rouge", cle: "d" + e.ordre, titre: `Dépense payée pour le client ${fmtF(e.montant)} — ${e.journal} ${e.piece ?? ""}`, detail: e.intitule ?? "" })),
    ...
    ...docs.map((d) => ({ date: d.genere_le as string, classe: "gris", cle: "doc" + d.id, titre: `${LIBELLES_DOCUMENT[d.type] ?? d.type}${d.numero ? ` ${d.numero}` : ""} arrêté au ${formatDate(d.date_arrete)}`, detail: d.chemin_ged ?? "" })),
  ].sort((x, y) => (x.date < y.date ? 1 : -1)) : [];
  const pageTl = Math.max(1, Number(sp.page ?? 1));
  const TAILLE = 100;
  ...
  return (
    <>
      <div className="fh">
        ...
        <span style={{ flex: 1 }} />
        {droitPointage ? <form action={genererSituation.bind(null, compte)}><button className="btn bl" type="submit">📄 Situation officielle</button></form> : <Link className="btn" href={`/clients/${compte}/situation`}>📄 Aperçu situation</Link>}
      ...
      {c.bv_bloque && <div className="warn">Compte BV : bons servis ({fmtF(c.bons_servis)}) supérieurs à {parametres.seuils.bv_ratio_max} × les règlements — pas de nouveau lot de bons sans règlement du précédent (règle DG).</div>}
      ...
      <div className="tuiles">
        <div className="tuile"><div className="l">Report à nouveau</div><div className="v" style={{ fontSize: 15 }}>{fmtF(c.ran)}</div></div>
        ...
        <div className="tuile v2"><div className="l">Réglé</div><div className="v" style={{ fontSize: 15 }}>{fmtF(c.regle)}</div><div className="d">{c.nb_reglements} règlement(s){c.nb_regularisations ? ` + ${c.nb_regularisations} régularisation(s)` : ""} · dernier {formatDate(c.dernier_reglement, true)}{c.jours_sans_reglement !== null ? ` (${c.jours_sans_reglement} j)` : ""}</div></div>
      ...
      <nav className="onglets" style={{ position: "static", marginBottom: 12, borderRadius: 8 }}>
        {[["synthese", "Synthèse"], ["reglements", `Règlements${besoinEcr ? ` (${regls.length})` : ""}`], ["livraisons", "Livraisons"], ["timeline", "Chronologie"], ["messages", "Messages"], ["fiche", "Fiche & contacts"]].map(([k, l]) => (
        ...
        <div className="grid2">
          <div className="card">
            ...
            <div className="tbl"><table>
              ...
              <tbody>
                {facturation.map((m) => <tr key={m.mois}><td>{libelleMois(m.mois)}{extraction?.saisi_jusquau && m.mois === extraction.saisi_jusquau.slice(0, 7) ? <span className="muted"> (saisi jusqu&apos;au {formatDate(extraction.saisi_jusquau, true)})</span> : ""}</td><td className="num">{fmt(litresMois.get(m.mois) ?? 0)}</td><td className="num">{fmt(m.ht)}</td></tr>)}
                {facturation.length === 0 && <tr><td colSpan={3} className="muted">Aucune facturation sur la période extraite.</td></tr>}
              </tbody>
          ...
          <div className="card">
            ...
            {ouvertes.map((a) => (
              <div key={a.id} className={`kcard ${a.type === "contentieux" || a.type === "mise_en_demeure" ? "rouge" : a.type === "promesse" || a.type === "plan" ? "or" : ""}`}>
                <b>{LIBELLES_TYPE_ACTION[a.type]}{a.niveau ? ` — niveau N${a.niveau}` : ""}{a.montant ? ` — ${fmtF(a.montant)}` : ""}{a.echeance ? ` — échéance ${formatDate(a.echeance)}` : ""}{a.resultat === "non_tenue" ? " — NON TENUE" : ""}</b>
                ...
                {a.type === "plan" && plans.filter((p) => p.action_id === a.id).map((p) => (
                  <div key={p.id} className="muted">échéance {formatDate(p.echeance)} : {fmtF(p.montant)} {p.tenue ? `✔ (${p.reglee_par_piece ?? "réglée"})` : ""}</div>
                ))}
                {droitReco && (
                  <form action={fermerAction.bind(null, compte, a.id)} className="frm">
                    {(a.type === "promesse" || a.type === "plan") && <select name="resultat" defaultValue={a.resultat === "non_tenue" ? "non_tenue" : "tenue"}><option value="tenue">Tenue</option><option value="non_tenue">Non tenue</option><option value="annulee">Annulée</option></select>}
            ...
            {droitReco && Number(c.solde) > -1000 && (
              <details className="mt">
                ...
                <form action={creerAction.bind(null, compte)} className="frm zone" style={{ flexDirection: "column", alignItems: "stretch" }}>
                  <div className="frm">
                    <label className="ch">Type<select name="type" defaultValue="relance">
                      ...
                      {profil.role === "dg" && <option value="mise_en_demeure">Mise en demeure (DG, N4)</option>}
                      {profil.role === "dg" && <option value="contentieux">Contentieux (DG, après mise en demeure)</option>}
                    </select></label>
                    ...
                    <label className="ch">Date<input type="date" name="date_action" defaultValue={aujourdhui()} max={aujourdhui()} /></label>
      ...
      {onglet === "reglements" && (
        ...
        <div className="card p0">
          ...
          <div className="tbl"><table>
            <thead><tr><th>Date</th><th>Journal</th><th>Pièce</th><th className="opt">Référence</th><th className="opt">Libellé</th><th>Payeur / lien</th><th className="num">Montant (F)</th>{droitPointage && <th>Qualifier</th>}</tr></thead>
            <tbody>
              {regls.length === 0 && <tr><td colSpan={8} className="muted">Aucun règlement sur la période extraite.</td></tr>}
              {regls.map((e) => (
                <tr key={e.ordre} style={{ opacity: e.est_regularisation ? 0.6 : 1 }}>
                  <td>{formatDate(e.date_ecriture)}</td><td>{e.journal}</td><td>{e.piece}</td><td className="opt">{e.ref_piece}</td><td className="muted opt" style={{ maxWidth: 260 }}>{e.intitule}</td>
                  <td>
                    {e.est_regularisation && <Badge classe="gris" title="Neutre sur le solde, hors cadence">régularisation</Badge>}{" "}
                    {multiSet.has(`${e.journal}|${e.piece}`) && <Badge classe="or" title="Même pièce sur plusieurs comptes">multi-clients</Badge>}{" "}
                    {e.payeur && <Badge classe="bleu">{e.payeur}</Badge>}{" "}
                    {(e.liens ?? []).map((l, i) => <Badge key={i} classe="or" title={l.note ?? ""}>{LIBELLES_LIEN[l.lien] ?? l.lien}{l.compte_lie ? ` ${l.compte_lie}` : ""}{l.piece_liee ? ` → ${l.piece_liee}` : ""}</Badge>)}
                  </td>
                  <td className="num"><b>{fmt(e.montant)}</b></td>
                  {droitPointage && <td>
                    <details><summary className="btn sm">✎</summary>
                      <form action={qualifierReglement.bind(null, compte)} className="frm zone" style={{ flexDirection: "column", alignItems: "stretch", minWidth: 220 }}>
                        <input type="hidden" name="piece" value={e.piece ?? ""} /><input type="hidden" name="journal" value={e.journal} /><input type="hidden" name="nature" value="reglement" />
            ...
            <tfoot><tr><td colSpan={6}>TOTAL CRÉDITS ({regls.length})</td><td className="num">{fmt(c.regle)}</td>{droitPointage && <td />}</tr></tfoot>
          </table></div>
          {(debits.length > 0 || rans.length > 0) && (
            <>
              <h3 style={{ padding: "12px 14px 0" }}>Autres écritures (RAN, dépenses payées pour le client)</h3>
              <div className="tbl"><table>
                <thead><tr><th>Date</th><th>Journal</th><th>Pièce</th><th className="opt">Libellé</th><th>Sens</th><th className="num">Montant (F)</th></tr></thead>
                <tbody>{[...rans, ...debits].map((e) => <tr key={e.ordre}><td>{formatDate(e.date_ecriture)}</td><td>{e.journal}</td><td>{e.piece}</td><td className="muted opt">{e.intitule}</td><td>{e.sens === 0 ? "débit" : "crédit"}</td><td className="num">{fmt(e.montant)}</td></tr>)}</tbody>
              </table></div>
            </>
          )}
        </div>
        {payeurs.length > 0 && (
          <div className="card p0"><h3>Soldes par payeur <small>compte collectif</small></h3>
            <div className="tbl"><table><thead><tr><th>Payeur</th><th className="num">Bons étiquetés</th><th className="num">Règlements étiquetés</th><th className="num">Solde</th></tr></thead>
              <tbody>{payeurs.map((p) => <tr key={p.payeur}><td>{p.payeur}</td><td className="num">{fmt(p.bons)}</td><td className="num">{fmt(p.reglements)}</td><td className="num"><b>{fmt(p.solde)}</b></td></tr>)}</tbody></table></div>
          </div>
        )}
        </>
      )}
      ...
      {onglet === "livraisons" && (
        <div className="card p0">
          <h3>Livraisons <small>{livr.length} lignes · le numéro de station fait foi</small></h3>
          <div className="tbl" style={{ maxHeight: "65vh", overflow: "auto" }}><table>
            <thead><tr><th>Date</th><th>Pièce</th><th className="opt">Produit</th><th>Bon / camion</th><th className="num">Litres</th><th className="num">Montant HT</th><th>Station</th><th>Payeur</th></tr></thead>
            <tbody>{livr.slice(0, 500).map((l, i) => <tr key={i}><td>{formatDate(l.date_livraison)}</td><td>{l.piece}</td><td className="opt">{l.ar_ref}</td><td>{l.designation}</td><td className="num">{fmt(l.qte)}</td><td className="num">{fmt(l.montant_ht)}</td><td>{l.station_numero ? <b>{l.station_numero}</b> : null} {l.station ?? l.depot}</td>
              <td>{l.payeur ?? ""}{droitPointage && <details style={{ display: "inline-block" }}><summary className="btn sm">✎</summary>
                <form action={qualifierReglement.bind(null, compte)} className="frm zone"><input type="hidden" name="piece" value={l.piece ?? ""} /><input type="hidden" name="journal" value="" /><input type="hidden" name="nature" value="bon" /><input name="payeur" placeholder="Payeur du bon" defaultValue={l.payeur ?? ""} /><button className="btn sm bl" type="submit">OK</button></form></details>}</td></tr>)}</tbody>
          </table></div>
          {livr.length > 500 && <p className="note" style={{ padding: "0 14px 12px" }}>{livr.length - 500} livraisons plus anciennes non affichées (voir la situation officielle ou l&apos;export).</p>}
        </div>
      )}

      {onglet === "timeline" && (
        <div className="card">
          <h3>Chronologie unifiée <small>{timeline.length} événements — datés et signés</small></h3>
          <div className="tl">
            {timeline.slice((pageTl - 1) * TAILLE, pageTl * TAILLE).map((ev) => (
          ...
          {timeline.length > TAILLE && <div className="frm">{pageTl > 1 && <Link className="btn sm" href={`/clients/${compte}?onglet=timeline&page=${pageTl - 1}`}>← Plus récents</Link>}<span className="muted">page {pageTl} / {Math.ceil(timeline.length / TAILLE)}</span>{pageTl * TAILLE < timeline.length && <Link className="btn sm" href={`/clients/${compte}?onglet=timeline&page=${pageTl + 1}`}>Plus anciens →</Link>}</div>}
        </div>
      )}
        ...
        <div className="grid2">
          <div className="card">
            ...
            <form method="get" className="frm">
              ...
              <select name="modele" defaultValue={sp.modele ?? ""}><option value="">— choisir un modèle —</option>{modeles.map((m) => <option key={m.code} value={m.code}>{m.libelle}{m.valide_par_dg ? "" : " (non validé DG)"}</option>)}</select>
          ...
          <div className="card">
            ...
            {messages.slice(0, 50).map((m) => <div key={m.id} className="kcard"><b>{formatDateHeure(m.envoye_le)} · {LIBELLES_CANAL[m.canal] ?? m.canal} → {m.destinataire ?? "—"} · {m.envoye_par_nom}</b><div className="pre" style={{ marginTop: 4 }}>{m.contenu}</div></div>)}
      ...
      {onglet === "fiche" && (
        <form action={enregistrerClientExt.bind(null, compte)} className="card">
          ...
          <div className="frm">
            ...
            {profil.role === "dg" ? <label className="ch">Limite de crédit (F, fixée par le DG)<input type="number" name="limite_credit" defaultValue={c.limite_credit ?? ""} min={0} step={1} /></label>
              : <label className="ch">Limite de crédit (DG)<input value={c.limite_credit ? fmt(c.limite_credit) + " F" : "non fixée"} readOnly /></label>}
